...
class KeyPair(object):
    def __init__(self, path=DefaultPath, view=None):
        ...
        while True:
            # first time, generate new keys natively using PyCryptodome
            if not os.path.exists(self.priv_path) or not os.path.exists(self.pub_path):
                self._view.on_keys_generation()
                logging.info("generating %s natively ..." % self.priv_path)
                ...
                # Secure the permissions so SSH/Systemd doesn't complain
